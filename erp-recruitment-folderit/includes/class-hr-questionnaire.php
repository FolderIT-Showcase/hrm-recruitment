...
class Hr_Questionnaire {
    ...
    function __construct() {
        ...
        $this->assign_type = array(
            ''                  => __('-- Select --', 'wp-erp-rec'),
            'all_employee'      => __('All Employees', 'wp-erp-rec'),
            'selected_employee' => __('Selected Employee', 'wp-erp-rec')
        );
    ...
    function post_types() {
        ...
        register_post_type( $this->post_type, array(
            'label'              => __( 'HR Questionnaire', 'wp-erp-rec' ),
            'description'        => '',
            'public'             => true,
            'show_ui'            => true,
            'show_in_menu'       => true,
            ...
            'labels'             => array(
                'name'               => __( 'HR Questionnaire', 'wp-erp-rec' ),
                'singular_name'      => __( 'HR Questionnaire', 'wp-erp-rec' ),
                'menu_name'          => __( 'HR Recruitment', 'wp-erp-rec' ),
                'add_new'            => __( 'Add HR Questionnaire', 'wp-erp-rec' ),
                'add_new_item'       => __( 'Add New HR Questionnaire', 'wp-erp-rec' ),
                'edit'               => __( 'Edit', 'wp-erp-rec' ),
                'edit_item'          => __( 'Edit HR Questionnaire', 'wp-erp-rec' ),
                'new_item'           => __( 'New HR Questionnaire', 'wp-erp-rec' ),
                'view'               => __( 'View HR Questionnaire', 'wp-erp-rec' ),
                'view_item'          => __( 'View HR Questionnaire', 'wp-erp-rec' ),
                'search_items'       => __( 'Search HR Questionnaire', 'wp-erp-rec' ),
                'not_found'          => __( 'No HR Questionnaire Found', 'wp-erp-rec' ),
                'not_found_in_trash' => __( 'No HR Questionnaire found in trash', 'wp-erp-rec' ),
                'parent'             => __( 'Parent HR Questionnaire', 'wp-erp-rec' )
            )
        ) );
    }
    ...
    public function do_metaboxes() {
        add_meta_box(
            ...
            __( 'Questionnaire Settings', 'wp-erp-rec' ),
    ...
    public function meta_boxes_cb( $post_id ) {
        ...
        <div id="questions-field-main" xmlns:v-bind="http://www.w3.org/1999/xhtml"
            ...
            <div id="add-new-field">
                <button class="button-primary" @click.prevent="addNewField"><?php _e('Add New Question', 'wp-erp-rec'); ?></button>
            </div>
        </div>
        ...
        <script type="text/template" id="single-field-template">
            ...
            <div class="extended-people-single" v-show="edit" transition="extended-single">
                <div class="extended-button-holder">
                    <h4 class="erp-left"><?php _e( 'Question No', 'wp-erp-rec' ); ?> #{{ count+1 }}</h4>
                ...
                <div class="extended-inside">
                    <div class="extended-inside-single">
                        <div class="extended-label">
                            <span><?php _e('Question', 'wp-erp-rec');?></span>
                        </div>
                        <textarea class="widefat regular-text input-question-text" v-model="model.label" placeholder="<?php _e( 'Question', 'wp-erp-rec' ); ?>"></textarea>
                    </div>
                    ...
                    <div class="extended-inside-single">
                        ...
                        <div class="extended-label">
                            <label>
                                ...
                                <span><?php _e('This is a required field', 'wp-erp-rec');?></span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="extended-inside-single">
                        <div class="extended-label">
                            <span><?php _e('Field Type','wp-erp-rec');?></span>
                        </div>
                    ...
                    <div class="extended-inside-single" v-show="hasChildOptions">
                        <div class="extended-child-options">
                            <div class="single-input-option" v-for="option in model.options" track-by="$index">
                                ...
                                <span class="button remove-option" style="cursor: pointer" v-on:click="removeOption(option)"><?php _e('Remove','wp-erp-rec');?></span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="extended-inside-single" v-show="hasChildOptions">
                        <div class="extended-child-options add-option" v-show="hasChildOptions">
                            <button class="button" @click.prevent="addNewOption(model)"><?php _e('Add Option','wp-erp-rec');?></button>
                        </div>
                    </div>

                    <div class="extended-inside-single">
                        <div class="extended-label">
                            <span><?php _e('Help Text','wp-erp-rec');?></span>
                        </div>
                        <textarea v-model="model.helptext" class="widefat regular-text" placeholder="<?php _e( 'Enter your helper text', 'wp-erp-rec' ); ?>"></textarea>
                    </div>
