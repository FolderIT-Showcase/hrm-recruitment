<div class="wrap uniq-wrap" id="uniq-wrap">
  ...
  <div class="postbox metabox-holder" style="padding-top: 0; max-width: 1060px; margin: 0 auto;">
    <h3 class="openingform_header_title hndle"><?php _e( 'Candidate basic information', 'wp-erp-rec' ); ?></h3>
    <div class="inside" style="overflow-y: hidden;">
      <form action="<?php echo admin_url( 'edit.php?post_type=erp_hr_recruitment&page=add-opening&action=edit&step=candidate_basic_information' ); ?>" method="post" id="unique-form-pages">
        <div class="openingform_input_wrapper">
          <div id="openingform_candidate_basic_information">
            <p class="info-message"><?php _e('The first 3 fields (Name, Email and Upload CV) are mandatory and will be displayed in candidate application form.', 'wp-erp-rec');?></p>

            <div class="applicant_require_personal_fields">
              <label>
                <?php _e('Name', 'wp-erp-rec');?>
              </label>
        ...
        <br style="clear: both">
        <a href="<?php echo admin_url( 'edit.php?post_type=erp_hr_recruitment&page=add-opening&action=edit&step=job_information&postid='.$postid ); ?>" class="button button-hero"><?php _e('&larr; Back', 'wp-erp-rec');?></a>
        <input type="submit" name="candidate_basic_information" class="button-primary button button-hero alignright" value="<?php _e( 'Next &rarr;', 'wp-erp-rec');?>">
      </form>
    </div>
  </div>
</div>