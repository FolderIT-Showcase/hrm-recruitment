...
(function ($) {
  ...
  var WeDevs_ERP_Recruitment = {
    ...
    initialize: function () {
      ...
      $('.file_upload').on('click', function (event) {
        ...
        file_frame.on('select', function () {
          ...
          fattachment = file_frame.state().get('selection').first().toJSON();
          // Do something with attachment.id and/or attachment.url here
          //reset attach then attach again
          $('#jobseeker_form .attacher_name').remove();
          $('#jobseeker_form .file_upload').after("<span class='attacher_name'>" + fattachment.filename + "</span>");
          $('#jobseeker_form .file_upload').after("<input type='hidden' class='attacher_name' name='attach_id' value='" + fattachment.id + "'>");
        });
      ...
      $('.erp-filter-status').on('change', function () {
        ...
        if (catFilter != '') {
          //document.location.href = 'admin.php?page=jobseeker_list'+catFilter;
      ...
      $('.btn-attach-cv:not(:disabled)').click(function () {
        $.erpPopupBs({
          ...
          id: 'upload-cv-top',
          content: wp.template('erp-rec-cv-upload-template')().trim(),
          extraClass: 'smaller',
          onReady: function (modal) {
            modal.enableButton();
            var application_id = WeDevs_ERP_Recruitment.getApplicationId();
            $('#attachment_application_id').val(application_id);
            var applicant_id = jQuery('#applicant_id').val();
            $('#attachment_applicant_id').val(applicant_id);
          },
          onSubmit: function (modal) {
            modal.disableButton();

            var form_data = new FormData();
            form_data.append('fdata', this.serialize());
            form_data.append('erp_rec_file', $('#erp_rec_file').prop('files')[0]);
            form_data.append('action', 'erp-rec-upload-cv');
            form_data.append('_wpnonce', wpErpRec.nonce);
      ...
      $('#todo-calendar-overview').load(ajaxurl, {
        action: 'erp-get-calendar-overview'
      }, function (res) {
      ...
      $('#section-overview').click(function () {
        ...
        if ($('#todo-calendar-overview div').hasClass('fc-toolbar') == false) {
          $('.erp-calendar-detail .spinner').css({
            'visibility': 'visible'
          });
          $('#todo-calendar-overview').load(ajaxurl, {
            action: 'erp-get-calendar-overview'
      ...
      $('#section-overdue').click(function () {
        ...
        if ($('#todo-calendar-overdue div').hasClass('fc-toolbar') == false) {
          ...
          $('#todo-calendar-overdue').load(ajaxurl, {
            action: 'erp-get-calendar-overdue'
          }, function (res) {
            $('.erp-calendar-detail .spinner').css({
              'visibility': 'hidden'
            });
      ...
      $('#section-today').click(function () {
        ...
        if ($('#todo-calendar-today div').hasClass('fc-toolbar') == false) {
          ...
          $('#todo-calendar-today').load(ajaxurl, {
            action: 'erp-get-calendar-today'
          }, function (res) {
            $('.erp-calendar-detail .spinner').css({
              'visibility': 'hidden'
            });
      ...
      $('#section-later').click(function () {
        ...
        if ($('#todo-calendar-later div').hasClass('fc-toolbar') == false) {
          $('#todo-calendar-later').load(ajaxurl, {
            action: 'erp-get-calendar-later'
          }, function (res) {
            $('.erp-calendar-detail .spinner').css({
              'visibility': 'hidden'
            });
      ...
      $('#section-no-due-date').click(function () {
        ...
        $.getJSON(ajaxurl, {
          action: 'erp-get-calendar-no-date'
        }, function (res) {
          $('.erp-calendar-detail .spinner').css({
            'visibility': 'hidden'
          });
      ...
      $('#section-this-month').click(function () {
        ...
        if ($('#todo-calendar-this-month div').hasClass('fc-toolbar') == false) {
          $('#todo-calendar-this-month').load(ajaxurl, {
            action: 'erp-get-calendar-this-month'
          }, function (res) {
            $('.erp-calendar-detail .spinner').css({
              'visibility': 'hidden'
            });
      ...
      if ($('#job-information-step').length > 0) {
        ...
        $('#job_information').click(function (e) {
          ...
          } else if (expire_date_value == '') {
            flag_for_success_validation = false;
            error_text_messaage = wpErpRec.information_validation_message.expire_date_validation_message;
          } else if (glocation == '') {
            flag_for_success_validation = false;
    ...
    jobSeekerFormSubmit: function (e) {
      ...
      $('#job_seeker_table_wrapper .spinner').css({
        'visibility': 'visible'
      });
